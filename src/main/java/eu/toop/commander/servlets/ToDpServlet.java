...
public class ToDpServlet extends HttpServlet {
  ...
  protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
    ...
    resp.getOutputStream().print("<html><head><title>Toop Commander</title></head><body><h2>to-dp endpoint</h2></body></html>");
  ...
  protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
    ...
    try {
      ...
      LOGGER.debug("Parse toop request");
      TDETOOPRequestType tdeToopRequestType = ToopMessageBuilder.parseRequestMessage(req.getInputStream());

      LOGGER.debug(tdeToopRequestType.getDataRequestSubject().toString());
